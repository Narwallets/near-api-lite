...
async function broadcast_tx_commit_actions(actions, signerId, receiver, privateKey) {
    ...
    do {
        ...
        if (retry > 0 || !cachedAccessKey || (Date.now() - cachedAccessKey.timestamp) > 30000) {
            ...
            if (logLevel >= 2)
                console.log(`Re-fetched from chain access key for ${signerId}: got nonce:${cachedAccessKey.nonce}`);
        }
        else {
            if (logLevel >= 2)
                console.log(`Using cached access key for ${signerId}: have nonce:${cachedAccessKey.nonce}`);
        }
        // converts a recent block hash into an array of bytes
        // this hash was retrieved earlier when creating the accessKey (Line 26)
        // this is required to prove the tx was recently constructed (within 24hrs)
        last_block_hash = (0, serialize_js_1.decodeBase58)(cachedAccessKey.block_hash);
