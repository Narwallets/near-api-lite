 ...
 */
function setDryRunMode(onOff) {
    dryRunMode = onOff;
}
exports.setDryRunMode = setDryRunMode;
//--helper fn
let last_block_hash;
let last_block_hash_timestamp = 0;
function setLastBlockHash(bh) {
    last_block_hash = bh;
    last_block_hash_timestamp = Date.now();
}
function lastBlockHashSeen() {
    return (0, serialize_js_1.decodeBase58)(last_block_hash);
}
...
async function queryAccount(accountId) {
    try {
        return await (0, json_rpc_js_1.jsonRpcQuery)({
...
function access_key(accountId, publicKey) {
    return (0, json_rpc_js_1.jsonRpcQuery)({
...
function viewRaw(contractId, method, params) {
    ...
    if (params)
        encodedParams = Buffer.from(JSON.stringify(params)).toString("base64");
    return (0, json_rpc_js_1.jsonRpcQuery)({
...
function broadcast_tx_commit_signed(signedTransaction) {
    ...
    return (0, json_rpc_js_1.jsonRpc)('broadcast_tx_commit', {
...
async function broadcast_tx_commit_actions(actions, signerId, receiver, privateKey) {
    ...
    if (!akCachedNonce || (Date.now() - akCachedNonce.timestamp) > 30000) {
        ...
        if (accessKeyInfo.block_hash)
            setLastBlockHash(accessKeyInfo.block_hash);
        if (accessKeyInfo.block_height)
            last_block_height = accessKeyInfo.block_height;
        ...
        if (logLevel >= 2)
            console.log(`Re-fetched ak from chain ${signerId}: got nonce:${akCachedNonce.nonce}`);
    }
    else {
        if (logLevel >= 2)
            console.log(`Using cached ak for ${signerId}, nonce:${akCachedNonce.nonce}`);
    }
    ...
    if (logLevel >= 2)
        console.log(`nonce++ for ${signerId} = ${nonce}`);
    ...
    do {
        ...
        if (Date.now() > last_block_hash_timestamp + 15000) {
            ...
            let blockInfo = await (0, json_rpc_js_1.jsonRpc)("block", { finality: 'final' });
            if (logLevel >= 2)
                console.log(`refresh-block-hash: ${(_a = blockInfo === null || blockInfo === void 0 ? void 0 : blockInfo.header) === null || _a === void 0 ? void 0 : _a.hash}`);
            if (blockInfo) {
                setLastBlockHash(blockInfo.header.hash);
                last_block_height = blockInfo.header.height;
            }
        }
        const transaction = TX.createTransaction(signerId, publicKey, receiver, nonce, actions, (0, serialize_js_1.decodeBase58)(last_block_hash));
        ...
        try {
            ...
            if (hashTooOldRetries < 2 && ex && ex.message && ex.message.includes("InvalidTxError:Expired")) {
...
function call(contractId, method, params, sender, privateKey, TGas, attachedYoctos) {
    if (dryRunMode || logLevel > 0) {
        console.log(`${dryRunMode ? "DRY-RUN " : ""} near call ${contractId} ${method} ${params instanceof Uint8Array ? "[Uint8Array]" : JSON.stringify(params)} --accountId ${sender} -gas:${TGas} --amount:${(0, conversion_js_1.yton)(attachedYoctos || "0")}`);
    }
    if (dryRunMode)
        return Promise.resolve(undefined);
