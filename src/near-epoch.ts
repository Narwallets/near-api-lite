import { block, BlockInfo, latestBlock } from "./near-rpc";
...
export async function computeCurrentEpoch(): Promise<EpochInfo> {
    ...
    const firstBlock = await block(lastBlock.header.next_epoch_id) //next_epoch_id looks like "current" epoch_id
    const prevBlock = await block(lastBlock.header.epoch_id) //epoch_id looks like "prev" epoch_id

    const epoch = new EpochInfo(prevBlock, firstBlock, lastBlock)
    console.log("estimated epoch duration in hours:", epoch.prev_epoch_duration_ms / HOURS)
...
export class EpochInfo {
    ...
    public prev_epoch_duration_ms: number;
    ...
    constructor(prevBlock: BlockInfo, startBlock: BlockInfo, lastBlock: BlockInfo) {
        ...
        if (this.length == 0) { //!prevBlock, genesis or hard-fork
            this.length = 43200;
            this.prev_epoch_duration_ms = 12 * HOURS;
    ...
    hours_to_end(): number {
        return Math.round((this.start_timestamp + this.prev_epoch_duration_ms - new Date().getTime()) / HOURS * 100) / 100;
    }
}
